...
static pmix_status_t ds21_init(pmix_info_t info[], size_t ninfo)
{
    ...
    if (PMIX_SUCCESS != (rc =_esh_session_init(tbl_idx, ns_map, _base_path, _jobuid,
                                ...
                                size_t ninfo)
{
    ...
    if (PMIX_SUCCESS != _esh_jobuid_tbl_search(jobuid, &tbl_idx)) {
        ...
        if (PMIX_SUCCESS != (rc =_esh_session_init(tbl_idx, ns_map, _base_path, jobuid, setjobuid, local_size))) {
            rc = PMIX_ERROR;
            PMIX_ERROR_LOG(rc);
            return rc;
        }
