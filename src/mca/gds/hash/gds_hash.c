...
pmix_status_t hash_cache_job_info_20(struct pmix_nspace_t *ns,
                                  pmix_info_t info[], size_t ninfo);
...
pmix_gds_base_module_t pmix_hash_module = {
    ...
    .cache_job_info = hash_cache_job_info_20,
...
static pmix_status_t store_map_20(pmix_hash_table_t *ht,
                        char **nodes, char **procs)
{
    ...
    PMIX_BFROPS_PACK(rc, pmix_globals.mypeer, &buf, &nnodes, 1, PMIX_SIZE);
    if (PMIX_SUCCESS != rc) {
        PMIX_ERROR_LOG(rc);
        return rc;
    }
    for (i=0; i <nnodes; i++) {
        ...
        kv->key = strdup(nodes[i]);
        kv->value->data.string = strdup(procs[i]);
    ...
    kv->key = strdup(PMIX_MAP_BLOB);
    kv->value = (pmix_value_t*)malloc(sizeof(pmix_value_t));
    kv->value->type = PMIX_BYTE_OBJECT;
    kv->value->data.bo.bytes = buf.base_ptr;
    kv->value->data.bo.size = buf.bytes_used;

    pmix_output_hexdump(1, pmix_globals.debug_output,
                        buf.base_ptr, buf.bytes_used);

    if (PMIX_SUCCESS != (rc = pmix_hash_store(ht, PMIX_RANK_WILDCARD, kv))) {
        PMIX_ERROR_LOG(rc);
        PMIX_RELEASE(kv);
        PMIX_DESTRUCT(&buf);
        return rc;
                                  ...
                                  pmix_info_t info[], size_t ninfo)
{
    ...
    for (n=0; n < ninfo; n++) {
        if (0 == strcmp(info[n].key, PMIX_NODE_MAP)) {
            ...
            if (NULL != procs) {
                if (PMIX_SUCCESS != (rc = store_map_20(ht, nodes, procs))) {
                    PMIX_ERROR_LOG(rc);
                    goto release;
                }
            }
            ...
            PMIX_BFROPS_PACK(rc, pmix_globals.mypeer, &buf2, &rank, 1, PMIX_PROC_RANK);
            if (PMIX_SUCCESS != rc) {
                PMIX_ERROR_LOG(rc);
                PMIX_RELEASE(nptr);
                PMIX_DESTRUCT(&buf2);
            ...
            for (j=1; j < size; j++) {
                ...
                PMIX_BFROPS_PACK(rc, pmix_globals.mypeer, &buf2, kp2, 1, PMIX_KVAL);
                if (PMIX_SUCCESS != rc) {
                    PMIX_ERROR_LOG(rc);
                    PMIX_RELEASE(nptr);
                    PMIX_DESTRUCT(&buf2);
            ...
            /* now add the blob */
            kp2->key = strdup(PMIX_PROC_BLOB);
            PMIX_BFROPS_PACK(rc, pmix_globals.mypeer, &buf2, kp2, 1, PMIX_KVAL);
            kp2->value->type = PMIX_BYTE_OBJECT;
