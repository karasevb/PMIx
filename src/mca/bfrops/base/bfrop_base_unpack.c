...
static size_t pmix_bfrop_unpack_flex(const char in_buf[], size_t buf_size,
                                     uint64_t *out_val, uint8_t *out_val_size)
{
    ...
    if (PMIX_UNLIKELY((buf_size-1) == idx &&
    ...
    *out_val_size = (hi_bit + shift_last)/8 + !!((hi_bit + shift_last) & (8-1));
    *out_val = value;
                                               ...
                                               pmix_data_type_t type)
{
    ...
    size_t val_sizeof;
    pmix_status_t rc;
    uint8_t unpack_val_size;
    size_t unpack_max_size;
    ...
    /* unpack the data */
    for (i = 0; i < (*num_vals); ++i) {
        unpack_max_size = buffer->pack_ptr - buffer->unpack_ptr;
        /* check remained buf size */
        if (unpack_max_size > PMIX_BFROPS_FLEX_BASE7_MAX_BUF_SIZE) {
            /* set max buf size for flex upacking */
            unpack_max_size = PMIX_BFROPS_FLEX_BASE7_MAX_BUF_SIZE;
        }
